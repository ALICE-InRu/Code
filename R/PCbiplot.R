PCbiplot <- function(PC,diff, x="PC1", y="PC2", colors=c('black', 'black', 'red', 'red')) {
  # PC being a prcomp object
  data <- data.frame(obsnames=row.names(PC$x), PC$x)
  data$Difficulty <- diff
  plot <- ggplot(data, aes_string(x=x, y=y)) + geom_text(alpha=.4, size=3, aes(label=Difficulty, color=Difficulty))
  ...
  plot <- plot +  xlab('First principal component ') + ylab('Second principal component ')
  plot 
}