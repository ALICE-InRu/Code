correlation.matrix <- function(df, method = 'spearman')
{
  ...
  levels(cor.dat$Var1) <- renamed
  levels(cor.dat$Var2) <- rev(renamed)
  return(cor.dat)
}

plot.correlation.matrix <- function(cor.dat){
  ## Plotting
  library(ggplot2)
  ...
  p=ggplot(cor.dat, aes(x=Var2, y=Var1, fill = value)) +
    geom_tile() +
...
summary(dat)
cor.dat = correlation.matrix(dat)
plot.correlation.matrix(cor.dat)
