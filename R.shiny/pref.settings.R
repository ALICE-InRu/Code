...
rho.statistic <- function(model){
  ...
  dat$NrFeat=getAttribute(dat$CDR,m,'NrFeat',F)
  dat$Model=getAttribute(dat$CDR,m,'Model',F)
  m=regexpr("(?<Problem>[a-z].[a-z]+).(?<Dimension>[0-9x]+).(?<Rank>[a-z]).(?<Track>[A-Z]{2}[A-Z0-9]+).(?<Bias>[a-z0-9]+).weights.time",model,perl=T)
  dat$ModelProblem=getAttribute(model,m,'Problem')
  dat$Bias=getAttribute(model,m,'Bias')
  dat$TimeIndependent=grepl('timeindependent',model)
  dat$Dimension=getAttribute(model,m,'Dimension')
  dat$Track=getAttribute(model,m,'Track')
