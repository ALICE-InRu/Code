...
function [xmin esWeights]=bruteforceESapproach(basename)

  % --------------------  Initialization --------------------------------  
  %% User defined input parameters (need to be edited)
  global numFeatures numJobs numMacs dim2 rawData
  numFeatures=13;
  numJobs=6;
  numMacs=5;
  dim2=numJobs*numMacs;
  load([basename '-rawtrain.mat']); % <- rawData  
  savename=[basename '-cmarun.mat'];
  
  %% CMA-ES search commences:
  strfitnessfct = 'applyweights';  % name of objective/fitness function
  N = (numFeatures+1)*dim2;   % number of objective variables/problem dimension
  xmean = rand(N,1);    % objective variables initial point
  %% betra að gefa MWR-vigtum strax meira vægi
  xmean(6:numFeatures+1:N)=100.*xmean(6:numFeatures+1:N);
  %%
