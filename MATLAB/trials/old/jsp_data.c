...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  double time, starttime, minvalue;
  double *phi;
  int numfeatures = 15;
  
  if (nrhs < 4) mexErrMsgTxt("usage: [makespan,xTime,trajectory,lookahead,properties] = jsp_data(p,sigma,xTime,steplength)");
  ...
  for (i = 0; i < steplength; i++) {
    ...
    if (i == (steplength-1)) { /* only do this once we have reacted the steplength requested */
